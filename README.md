...
*   [安装教程](#安装教程)
    ...
    *   [一、刷入带有UA2F的固件](#一刷入带有ua2f的固件)
        ...
        *   [2.进入 Breed Web 恢复控制台](#2进入-breed-web-恢复控制台此处以新三为例其他路由器请自行百度)
    ...
    *   [二、路由器设置](#二路由器设置)
        ...
        *   [2.配置lan口](#2配置lan口可选若路由器出现网页加载慢但app可以正常访问网络的情况就配置)

        *   [3.配置WIFI](#3配置wifi)

        *   [4.配置管理界面（SSH）密码](#4配置管理界面ssh密码)
...
2.  主要为了应对2022年韶关学院新上网认证，目前只支持搭载OpenWrt的路由器配置

3.  并且Openwrt需已安装curl或wget模块，未安装的设备请先自行安装该模块

4.  您也可以fork本项目，并添加对其他系统的支持

5.  SGU-Script会实时监控您的网络状况，在掉线的时候进行自动重连，但并不能阻止共享上网的检查

6.  由于阻止共享上网的监测较为复杂，此处提供一个笔记以解决共享上网的解决方案（首推UA2F）

    [OpenWrt 编译与防检测部署教程](https://www.notion.so/OpenWrt-f59ae1a76741486092c27bc24dbadc59)

7.  本教程我也会致力于为大家解决校园网检测问题，我选择的是UA2F的方案

8.  我会在`firmware分支`中提前准备我适配好带UA2F的固件，但我不可能做到所有设备都适配，希望大家能够编译更多的固件放在`firmware分支`，然后给我提一个Pr

9.  下文安装教程也会教你如何安装固件

10. 若您不需要绕过校园网检测的话，直接看【安装教程】-【路由器设置】&&【安装SGU-Script】即可
...
Eg：当然如果你的有UA2F的ipk包可用，也可以直接安装ipk包，但是UA2F挑内核，一般会安装失败

### 1.刷入 Breed Web 恢复控制台（请自行百度）

*   已刷入 Breed Web 恢复控制台的请继续下一步

### 2.进入 Breed Web 恢复控制台（此处以新三为例，其他路由器请自行百度）

*   将网线的一端插在lan口上（黄色的网口），另一端插电脑

*   在`电源线`的旁边有一个小小的按钮，记住它（下文中我们称之为`RESET键`）

*   拔掉`电源线`，按下`RESET键`（不松开）
...
Eg：在`firmware分支`中有我提前编译好带UA2F的固件，若你使用的是该分支中的固件，请你提前先仔细阅读固件同文件夹下的`info.txt`，不同固件以下步骤可能会有所不同
...
### 2.配置lan口(可选，若路由器出现网页加载慢，但app可以正常访问网络的情况就配置)
...
*   可跨平台，只需拥有`jre`环境，使用`java -jar SGU-Script.jar`命令来启动程序
...
*   全部设置完成后，请将网线把wan口（即蓝色的口，但有些固件的接口是乱的，视具体情况而定）与墙上的网口连起来

### 4.开始享受

*   维护网络安全，保护个人隐私；妥善运用及保护个人信息，避免因个人信息泄露而让犯罪分子有机可乘；树文明新风，建和谐校园。
