...
2.  阅读`README.md`说明来选择适配你路由器的固件，若你使用的是该分支中的固件，请你先仔细阅读固件所在文件夹下的`info.txt`，不同固件以下步骤可能会有所不同
...
3.  进入wan口配置页面，可从菜单栏进入，也可以直接点击wan口右边的[修改]按钮，我们将协议切换为静态地址，并点击[切换协议]
...
4.  依次填入IP地址，子网掩码，网关（这三个值在统一认证平台中可以查到，在网管保修那里），DNS推荐使用阿里和腾讯的，`223.5.5.5`，`119.29.29.29`,有的DNS设置会在[高级设置]里，请自行查找
    ...
    ![输入图片说明](https://foruda.gitee.com/images/1666682686329269361/c711adbb_9532490.png "8-min.png")
