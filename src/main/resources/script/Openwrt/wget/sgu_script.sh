...
login() {
  if ping -c 2 www.baidu.com >/dev/null 2>&1; then
    ...
    response=$(wget --post-data="userId=$username&password=$password&queryString=wlanuserip=$ip&wlanacname=NAS&ssid=Ruijie&nasip=172.16.253.90&mac=000000000000&t=wireless-v2-plain&url=http://1.1.1.1/" http://172.16.253.93:8080/eportal/InterFace.do?method=login -qO-)
    if [ -n "$response" ]; then
